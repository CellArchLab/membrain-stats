...
from membrain_stats.utils.io_utils import get_mesh_filenames, get_mesh_from_file, get_tmp_edge_files
...
def protein_concentration_folder(
    ...
    meshes = [
        trimesh.Trimesh(
            vertices=mesh_dict["verts"] * (1.0 if pixel_size_multiplier is None else pixel_size_multiplier), 
    ...
    if exclude_edges:
        ...
        meshes = [
            get_edge_mask(
                ...
                temp_file=filename
                )[0] for mesh, filename in zip(meshes, out_files_edges)
                ]
    ...
    protein_concentrations = [concentration * 100 for concentration in protein_concentrations] # get value in nm^-2
